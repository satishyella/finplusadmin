 <div class="content">
	...
	?>
					<button><a href="#"   data-toggle="modal" data-target=".bs-example-modal-sm" data-backdrop="static" data-keyboard="false">Add Company</a></button>
...
<?PHP echo  $this->session->flashdata('susmsg')? $this->session->flashdata('susmsg'):'';?></font>
        ...
        <tbody>
		<?php  $sno=0; 
		foreach ($petdetails as $details){ $sno++?>
		 <tr>
                <td><?=$sno?></td>
                <td><?=$details['cik']?> </td>
                <td><?=$details['cname']?></td>
                <td><?=$details['firmid']?></td>
                <td><?=$details['comments']?></td>
                <td><?php if($details['status']){ echo '<a href='.site_url().'admin/act_inact_comp/'.$details['sno'].'/0/'.$this->session->userdata("token").'>Active</a>';} else { echo '<a href='.site_url().'admin/act_inact_comp/'.$details['sno'].'/1/'.$this->session->userdata("token").'>In-active</a>'; } ?></td>
